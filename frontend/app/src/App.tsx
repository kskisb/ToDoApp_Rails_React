...
const App = () => {
  ...
  return (
    <VStack spacing={4} padding={4}>
      ...
      <Flex>
        <Input placeholder='Todoを入力' value={title} onChange={(e) => setTitle(e.target.value)} />
        <Button colorScheme='blue' mx={4}><AddIcon onClick={handleCreateTodo} /></Button>
      </Flex>
      {todos.map((todo) => (
        <Box key={todo.id} display='flex' flexDirection='row' alignItems='center'>
